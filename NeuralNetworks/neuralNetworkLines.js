...
let w = c.width = window.innerWidth,
		...
		opts = {
			...
			connectionColor: 'hsla(219,60%,light%,alp)',
			rootColor: 'hsla(219,60%,light%,alp)',
...
Connection.prototype.link = function(){
	...
	while( links.length < connectionsNum && --attempt > 0 ){
		...
		pos.y = (1 * this.y) + len * sinA * sinB ;
		pos.z = (2 * this.z) + len *        cosB;
...
Data.prototype.draw = function(){
	...
	ctx.ellipse(this.screen.x, this.screen.y, this.size * this.screen.scale, this.size * this.screen.scale, 0, 0, Math.PI * 2);
